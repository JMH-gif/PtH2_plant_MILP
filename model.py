...
import time
import pandas as pd
from model_helper_functions import prepare_parameters, calculate_financial_results_from_results, calculate_cost_coverage_constraint
...
def setup_model(global_params=None, time_varying_scenario=None, generate_tables=False, generate_plots=False):
    ...
    # Slack variable for purchased electricity
    elec_purchased = model.addVars(T, name="elec_purchased")
    ...
    # High cost per unit of purchased electricity to discourage usage
    high_cost_per_unit = 1000000000000000000000000000000000000  # Set a very high cost per unit to penalize purchasing
    ...
    model.setObjective(
        hydrogen_price + gp.quicksum(elec_purchased[t] * high_cost_per_unit for t in T),
        GRB.MINIMIZE
    )
    ...
    slack_variable_cost = scaling_factor * gp.quicksum(elec_purchased[t] * high_cost_per_unit for t in T)

    calculate_cost_coverage_constraint(T, model,Annuity_factor_grid,yearly_hydrogen_revenue,o2_revenue,heat_revenue,yearly_grid_sale_revenue, yearly_transportation_cost, yearly_OPEX_fixed, yearly_CAPEX_cost,water_cost,slack_variable_cost,startup_cost_electrolyzer,u_electrolyzer_start,grid_connection_capacity,el_grid_connection_fee)
    ...
    if include_el_grid_connection_fee:
        ...
        model.addConstrs((elec_purchased[t] <= grid_connection_capacity for t in T), name="GridConnectionLimit")
    #                                        STORAGE CONSTRAINTS
    #--------------------------BATTERY:
    #
   ...
   # Link the initial and final SOCs to this variable for the battery
    ...
    model.addConstrs((x_out['battery', t] <= config.c_rate * aux_battery_discharge_cap[t] for t in T), name="C_Rate_DischargeLimit")
    # Constraint 1: Limit charging rate based on the C-rate and binary variable
    model.addConstrs((x_in['battery', t] <= config.c_rate * aux_battery_charge_cap[t] for t in T), name="C_Rate_ChargeLimit")
    ...
    model.addConstrs(
        (
            el_prod['wind', t] + el_prod['solar', t] + x_out['battery', t] + elec_purchased[t] ==
    ...
    if model.status == GRB.OPTIMAL:
        # Check if high_cost_per_unit was triggered
        high_cost_triggered = any(elec_purchased[t].x > 0 for t in T)
        if high_cost_triggered:
            print( "Warning: The high_cost_per_unit constraint was triggered. The model had to purchase electricity at a high cost, which may indicate infeasibility or high costs in certain periods.")
        ...
        results = {
            ...
            'battery_max_discharge': P_invest['battery'].x * config.c_rate*Eff['battery']['discharge'],
            'electrolyzer_max_intake': P_invest['electrolyzer'].x
        }
        print(P_invest['electrolyzer'].x*0.2)
        print("Test change for Git commit and push!")
        ...
        print(grid_connection_capacity.x)
        ...
        # Identify high-cost electricity purchase times
        high_cost_times = [t for t in T if elec_purchased[t].x > 0]
