...
h2_demand = 20  # kt/year


def hydrogen_model(day_ahead_price: list, eta_elecrolyser: float, h2_revenue: float, P_solar=cf_pv, P_wind=cf_wind,
                   CAPEX_wind=CAPEX_wind, CAPEX_solar=CAPEX_solar,
                   CAPEX_electrolyser=CAPEX_electrolyser, Bat_cap_min=SOC_min,
                   Bat_cap_max=SOC_max, Int_a=time_interval_start, Int_b=time_interval_stop,
                   CAPEX_Battery=CAPEX_Battery) -> float:
    ...
    C_battery = m.addVar(lb=0, name='Capacity battery', vtype=GRB.INTEGER)
    ...
    Bat_ch_dis = m.addVars(time, name='x_Bat_ch_dis', vtype=GRB.BINARY)
    ...
    obj_fct = quicksum(x_el_sold[t] * day_ahead_price[t] for t in time) - (CAPEX_wind * C_wind) - (
                CAPEX_solar * C_pv) - (CAPEX_electrolyser * C_electrolyser) - (CAPEX_Battery * C_battery) + quicksum(
        x_h2[t] * h2_revenue for t in time)
    ...
    for t in time:
        ...
        c2 = m.addConstr(x_h2[t] + x_el_sold[t] + Bat_ch[t] - Bat_disch[t] <= P_wind[t] * C_wind + P_solar[t] * C_pv,
                         name="energy_used_stays_leq_than_energy_available")
        # BATTERY CONSTRAINTS
    ...
    if plot == True:
        ...
        plt.axhline(y=Bat_cap_max, c='orange', linestyle='--', alpha=0.5)
        plt.show()
        """
